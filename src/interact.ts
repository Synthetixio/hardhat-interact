...
.addOptionalParam('driver', 'Format to export staged transactions. Either `gnosis-safe` or `curl` are currently supported.')
.addOptionalParam('out', 'Path which staged transactions should be written to')
...
async function executeSelection(ctx: InteractContext, batch: boolean) {
    ...
    if (!functionInfo.constant) {
        ...
        if (ctx.stagedTransactionDriver && ctx.stagedTransactionOutputFile) {
            // write a staged transaction
            appendFileSync(ctx.stagedTransactionOutputFile, `${ctx.stagedTransactionDriver(txn)}\n`);

            if (!batch)
                console.log(`> staged transaction appended to ${ctx.stagedTransactionOutputFile}`)
        }
        else if (!(ctx.signer instanceof Ethers.VoidSigner)) {
